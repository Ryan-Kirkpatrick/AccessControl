/*
    ...
    Contribs: pelrun (Sane rfid reading), jabeone (fix reset on some card reads bug), Ryan-Kirkpatrick (RFID reader fix, port to PlatformIO)
...
const char* ssid = "HSBNEInfra"; // Wifi SSID
const char* password = ""; // Wifi Password
const char* host = "https://portal.hsbne.org"; // Host URL
const char* secret = ""; // Secret to talk to the Host on.
const char* deviceName = "DOOR-Front"; // Device name. DOOR-DoorName or INT-InterlockName
const char* devicePassword = ""; // Password for OTA on device.
const char* deviceType = "door";
uint8_t contact = 1; // Set default switch state, 1 for doors that are permanantly powered/fail-open.
...
void IRAM_ATTR idleHeartBeatFlag() {
  triggerFlag = 1;
}



void IRAM_ATTR log(String entry) {
#ifdef SERIALDEBUG
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (doc["success"].as<String>() == "true") {
        ...
        if (serverCacheHash != curCacheHash) {
          ...
          triggerFlag = 0;
          log("[DEBUG] Idle: triggerFlag = 0");
        }
      } else {
        triggerFlag = 4;
        log("[DEBUG] Heartbeat fail. triggerFlag = 4");
      }
...
void setup() {
  ...
  http.on("/bump", []() {
    if (deviceType == "door") {
      ...
      log("[DEBUG] I'm not a door. I can not be bumped.");
    }
  });
#endif
  http.on("/checkin", []() {
